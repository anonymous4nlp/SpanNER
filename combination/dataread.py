...
class DataReader():
			...
			if '-DOCSTART-' in line:
				...
				for i, span1 in enumerate(spans1):
					if i == 0:  # skip the sentence content.
						...
						sid, eid = seids.split(',')
						# if ttag=='creative-work':
						# 	ttag='work'
						# if ptag == 'creative-work':
						# 	ptag = 'work'
	...
	def get_allModels_pred(self,):
		...
		# make the true chunk to a idx2label dic.
	...
	def get_sent_word(self):
		# choose CnnWglove_lstmCrf model as the standard
	...
	def read_span_score(self, pref_upchunks,fn_prob):
		test_word_sequences_sent = self.get_sent_word()
		...
		for num, fchunk in enumerate(pref_upchunks):
			...
			# print("new_all_predicts[sid]: ",new_all_predicts[sid])
		...
		# print("sent_lab_prb_dic: ", sent_lab_prb_dic)
