...
from datasets.dataload import BERTNERDataset
from datasets.truncate_dataset import TruncateDataset
from datasets.collate_functions import collate_to_max_length
from models.bert_spanPred_model import BertNER
from models.spanPred_config import BertNerConfig
from loss import *
from utils.get_parser import get_parser
from utils.radom_seed import set_random_seed
from metrics.functional.f1_metric import span_f1,span_f1_spanPred,\
	write_predict_spanPred,write_predict_spanPred_pruning,span_f1_spanPred_pruning,get_predict_spanPred,get_predict_spanPred_prune
import logging
...
class BertNerTagger(pl.LightningModule):
    ...
    def __init__(
        ...
        self.use_allspan = args.use_allspan


        self.max_span_len = args.max_span_len

        # for loss
        self.loss_func = torch.nn.MarginRankingLoss(self.margin)

        self.batch_size = args.batch_size
        self.n_samps = self.batch_size // self.n_class
        if self.batch_size % self.n_class != 0:
            print("self.batch_size: ", self.batch_size)
            print("self.n_class: ", self.n_class)
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("error! the batch_size should be the times of n_class.")
        ...
        # args.fwrite_epoch_res = fwrite_epoch_res
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--dataname", default="conll03",
                            help="data name")
        parser.add_argument("--max_span_len", type=int, default=4, help="max length of dataset")
        ...
        parser.add_argument('--use_morphology', type=str2bool, default=True, help='use the span length as a feature',
                            nargs='?',
                            choices=['yes (default)', True, 'no', False])
        ...
        parser.add_argument('--use_pred_pruning', type=str2bool, default=True,
                            help='best_dev_f1 value', )
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print("pos_span_mask_ltoken in train_step: ", pos_span_mask_ltoken)
        # print("words: ", words)
        ...
        output = {}
        if self.args.use_pred_pruning:
            span_f1s = span_f1_spanPred_pruning(all_span_idxs, predicts, span_label_ltoken, real_span_mask_ltoken,
                                            words,
                                            all_span_word, flat=True)
        else:
            span_f1s = span_f1_spanPred(self.n_samps, self.n_class, predicts, span_label_ltoken, real_span_mask_ltoken,
                                        words,
                                        all_span_word, flat=True)
        output["span_f1s"] = span_f1s
        rank_loss = self.compute_loss(loadall,all_span_rep, span_label_ltoken, real_span_mask_ltoken,mode='train')
        output[f"train_loss"] = rank_loss

        tf_board_logs[f"rank_loss"] = rank_loss

        output['loss'] = rank_loss
        output['log'] =tf_board_logs

        return output

        # return {'loss': rank_loss, 'log': tf_board_logs}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # num_tasks * [bsz, length, num_labels]
        # print("pos_span_mask_ltoken in validation_step: ", pos_span_mask_ltoken)
        # print("words: ", words)
        ...
        if self.args.use_pred_pruning:
            span_f1s,pred_label_idx = span_f1_spanPred_pruning(all_span_idxs, predicts, span_label_ltoken, real_span_mask_ltoken,
                                            words,
                                            all_span_word, flat=True)
            ...
            batch_preds = get_predict_spanPred_prune(self.args, all_span_word, words, pred_label_idx, span_label_ltoken,
                                               all_span_idxs)
        else:
            span_f1s = span_f1_spanPred(self.n_samps, self.n_class, predicts, span_label_ltoken, real_span_mask_ltoken,
                                        words,
                                        all_span_word, flat=True)
            batch_preds = get_predict_spanPred(self.args, all_span_word, words, predicts, span_label_ltoken,
                                               all_span_idxs)

        output["span_f1s"] = span_f1s
        rank_loss = self.compute_loss(loadall,all_span_rep, span_label_ltoken, real_span_mask_ltoken,mode='test/dev')


        output["batch_preds"] =batch_preds
        # output["batch_preds_prune"] = pred_label_idx_new
        output[f"val_loss"] = rank_loss
    ...
    def get_dataloader(self, prefix="train", limit: int = None) -> DataLoader:
        ...
        if not 'roberta' in self.args.bert_config_dir:
            vocab_path = os.path.join(self.bert_dir, "vocab.txt")
            ...
            print("use ByteLevelBPETokenizer as the tokenizer ")
            # tokenizer = Tokenizer.from_file(tokenizer_path)
            merge_path = os.path.join(self.bert_dir, "merges.txt")
            # sep_token = [SEP], cls_token = [CLS]
            tokenizer = ByteLevelBPETokenizer(vocab_path, merge_path,add_prefix_space=True)
